...
function Paginate() {
  ...
  Object.assign(Paginate.prototype, {
    getCurrentPage: function getCurrentPage() {
      return this._currentPage;
    },
    setCurrentPage: function setCurrentPage(ind) {
      return this._currentPage = ind;
    },
    _optionsHandler: function _optionsHandler(options) {
    ...
    _isValidElementSelector: function _isValidElementSelector(selector) {
      try {
    ...
    _selectArrayofElement: function _selectArrayofElement(selector) {
      if (this._isValidElementSelector(selector)) {
        if (document.querySelectorAll(selector).length != 0) return document.querySelectorAll(selector);else throw Error(`Your site doesn't contain any '${selector}' selector. Please provide a valid selector.`);
      } else throw Error(`'${selector}' is not a valid css selector Please provide a valid one.`);
    },
    _selectSingleElement: function _selectSingleElement(selector) {
      if (this._isValidElementSelector(selector)) {
        if (document.querySelector(selector)) return document.querySelector(selector);else throw Error(`Your site doesn't contain any '${selector}' selector. Please provide a valid selector.`);
      } else throw Error(`'${selector}' is not a valid css selector Please provide a valid one.`);
    },
    _checkElementOverflows: function _checkElementOverflows(element) {
    ...
    _pageItemsHorizontalScrollEventCallback: function _pageItemsHorizontalScrollEventCallback(e) {
      e.target.parentElement.scrollLeft += e.deltaY * 3;
    },
    _pageItemsHorizontalScroll: function _pageItemsHorizontalScroll(container) {
    ...
    _removePageItemsHorizontalScroll: function _removePageItemsHorizontalScroll(container) {
      if (container.classList.contains("row_view")) {
        const that = this;
        container.removeEventListener("wheel", that._pageItemsHorizontalScrollEventCallback);
      }
    },
    _pageItemsTouchScroll: function _pageItemsTouchScroll(container) {
    ...
    _removePageItemsTouchScrollEvent: function _removePageItemsTouchScrollEvent(container) {
      container.removeEventListener("mousedown", this._mouseDownHandler);
    },
    _handlePageScrollandTouchScrollDependingOverflows: function _handlePageScrollandTouchScrollDependingOverflows(container) {
    ...
    _createPagination: function _createPagination() {
      ...
      const more = this.defaults.hasEllips ? `<li class="pagination_item ellipse more"><span>...</span></li>` : "";
      const less = this.defaults.hasEllips ? `<li class="pagination_item ellipse less"><span>...</span></li>` : "";
      const first = !this.defaults.showFirstLast ? "" : `<li class="pagination_item paginaiton_first ${this.defaults.defaultClass}"><a href="" aria-label="${this.defaults.firstAreaLabel}" onclick="return false;">${this.defaults.firstAreaLabel}</a></li>`;
      const last = !this.defaults.showFirstLast ? "" : `<li class="pagination_item paginaiton_last ${this.defaults.defaultClass}"><a href="" aria-label="${this.defaults.lastAreaLabel}" onclick="return false;">${this.defaults.lastAreaLabel}</a></li>`;
      const previous = !this.defaults.showPrevNext ? "" : `<li class="pagination_item paginaiton_prev ${this.defaults.defaultClass}"><a href="" aria-label="${this.defaults.prevAreaLabel}" onclick="return false;">${this.defaults.prevAreaLabel}</a></li>`;
      const next = !this.defaults.showPrevNext ? "" : `<li class="pagination_item paginaiton_next ${this.defaults.defaultClass}"><a href="" aria-label="${this.defaults.nextAreaLabel}" onclick="return false;">${this.defaults.nextAreaLabel}</a></li>`;
        ...
        switch (element) {
          ...
          case "num":
            ...
            while (count <= this.pageCount - 1) {
              ...
              paginationContainer += `<li class="pagination_item ${this.defaults.defaultClass} data-item ${currentLink}" aria-label="${count}"><a href="#" onclick="return false">${Array.isArray(this.defaults.pageLinkAreaLabel) ? this.defaults.pageLinkAreaLabel[count] || count + 1 : count + 1}</a></li>`;
              count++;
            }
    ...
    _hideAllShowSpecificPaginationItems: function _hideAllShowSpecificPaginationItems(allEle) {
    ...
    _makePaginationBtnDisable: function _makePaginationBtnDisable() {
      // handle less ellipse
    ...
    _setActiveLink: function _setActiveLink(allEle) {
    ...
    _firstClickHandler: function _firstClickHandler(allItems, container) {
      const firstLink = this._selectSingleElement(".pagination_wrapper .pagination_item.paginaiton_first");
      const that = this;
    ...
    _prevClickHandler: function _prevClickHandler(allItems, container) {
      const prevLink = this._selectSingleElement(".pagination_wrapper .pagination_item.paginaiton_prev");
      const that = this;
    ...
    _ellipseMoreClickHandler: function _ellipseMoreClickHandler(allItems, container) {
      const ellipseMore = this._selectSingleElement(".pagination_wrapper .pagination_item.ellipse.more");
      const that = this;
    ...
    _ellipseLessClickHandler: function _ellipseLessClickHandler(allItems, container) {
      const ellipseLess = this._selectSingleElement(".pagination_wrapper .pagination_item.ellipse.less");
      const that = this;
    ...
    _numElemClickHandler: function _numElemClickHandler(allItems, container) {
    ...
    _nextClickHander: function _nextClickHander(allItems, container) {
      const nextLink = this._selectSingleElement(".pagination_wrapper .pagination_item.paginaiton_next");
      const that = this;
    ...
    _lastClickHandler: function _lastClickHandler(allItems, container) {
      const lastLink = this._selectSingleElement(".pagination_wrapper .pagination_item.paginaiton_last");
      const that = this;
  ...
  function ProPaginate(options) {
    ...
    this.defaults = Object.assign(this.defaults, {
      ...
      dataItemHtml: '<div class="data_item"><div class="data_item_details"><span class="data_item_date">${_date}</span><span class="data_item_location">${location}</span></div></div>',
  ...
  ProPaginate.prototype = Object.create(Paginate.prototype);
  Object.assign(ProPaginate.prototype, {
    _init: function _init() {
      ...
      if (this._isDataValid()) {
        this.mainContainer.forEach(function (mainElement) {
          ...
          const allPaginationLinkElements = this._selectArrayofElement("#pagination_container .data-item");
          ...
          const pageItemsContainer = this._selectSingleElement(".paginate_wrapper .items_container");
    ...
    _stringToTamplateGet: function _stringToTamplateGet(path, obj, fb = `$\{${path}}`) {
      return path.split(".").reduce((res, key) => res[key] || fb, obj);
    },
    _stringToTamplateParser: function _stringToTamplateParser(template, map, fallback) {
    ...
    _isDataValid: function _isDataValid() {
      if (!Array.isArray(this.defaults.data)) throw Error("your data should be an array.");else if (this.defaults.data.length == 0) throw Error("please provide some data.");else return true;
    },
    _createRenderPageItems: function _createRenderPageItems(container, currentpage) {
      let dataArr = this.defaults.data[currentpage][this.defaults.dataItemsArrayPath];
      let renderToCount = this.defaults.showAlldataOnce ? dataArr.length : Math.min(this.defaults.visibleDataonce, dataArr.length);
      let allElement = "";
      for (let i = 0; i < renderToCount; i++) {
        const htmlElement = this._stringToTamplateParser(this.defaults.dataItemHtml, dataArr[i]);
      ...
      if (dataArr.length > this.defaults.visibleDataonce) {
        if (!this.isShowMoreSectionExist) {
          ...
          const showMoreSection = this._selectSingleElement(".paginate_wrapper .show_more_btn");
          showMoreSection.addEventListener("click", this._showMoreClickEventCallback.bind(this));
        }
      } else {
        if (this.isShowMoreSectionExist) {
          const showMoreSection = this._selectSingleElement(".paginate_wrapper .show_more_btn");
          showMoreSection.removeEventListener("click", this._showMoreClickEventCallback.bind(this));
          showMoreSection.remove();
          this.isShowMoreSectionExist = false;
        }
      }
    },
    _createRenderShowMore: function _createRenderShowMore(container) {
    ...
    _showHideShowMoreScrollEventCallback: function _showHideShowMoreScrollEventCallback(e) {
      if (this.isShowMoreSectionExist) {
        const showMoreSection = this._selectSingleElement(".paginate_wrapper .show_more_btn");
    ...
    _showHideShowMore: function _showHideShowMore(container) {
      if (this._checkElementOverflows(container)) {
    ...
    _addMorePageItems: function _addMorePageItems(itemCount) {
      const pageItemsContainer = this._selectSingleElement(".paginate_wrapper .items_container");
      const alreadyExistEleCount = pageItemsContainer.querySelectorAll("div.data_item").length;
      const dataArr = this.defaults.data[this._currentPage][this.defaults.dataItemsArrayPath];
      const loopEnd = dataArr.length - alreadyExistEleCount < itemCount ? dataArr.length - 1 : alreadyExistEleCount - 1 + itemCount;
      for (let i = alreadyExistEleCount - 1; i < loopEnd; i++) {
        const newElements = this._stringToTamplateParser(this.defaults.dataItemHtml, dataArr[i]);
    ...
    _showMoreClickEventCallback: function _showMoreClickEventCallback(e) {
