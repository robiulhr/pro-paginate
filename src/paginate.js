...
class Paginate {
  ...
  _selectArrayofElement(selector) {
    if (this._isValidElementSelector(selector))
      if (document.querySelectorAll(selector).length != 0) return document.querySelectorAll(selector);
      else throw Error(`Your site doesn't contain any '${selector}' selector. Please provide a valid selector.`);
        ...
        case "num":
          ...
          if (this.pageCount > 1) {
            ...
            while (count <= this.pageCount - 1) {
              ...
              paginationContainer += `<li class="pagination_item ${this.defaults.defaultClass} data-item ${currentLink}" aria-label="${count}"><a href="#" onclick="return false">${
                Array.isArray(this.defaults.pageLinkAreaLabel) ? this.defaults.pageLinkAreaLabel[count] || count + 1 : count + 1
              }</a></li>`;
              count++;
            }
          }
  ...
  _hideAllShowSpecificPaginationItems(allEle) {
    if (this.pageCount > this.defaults.visiblePageCount) {
      allEle.forEach(function (item, ind) {
        if (ind < this.defaults.startpageNum || ind > this.defaults.startpageNum + (this.defaults.visiblePageCount - 1)) item.style.display = "none";
  ...
  _setActiveLink(allEle) {
    allEle.forEach(function (elem, ind) {
      if (this._currentPage == ind) elem.classList.add(this.defaults.activeClass);
...
class ProPaginate extends Paginate {
  ...
  _isDataValid() {
    if (!Array.isArray(this.defaults.data)) throw Error("your data should be an array.");
    else return true;
  }
  ...
  _showMoreClickEventCallback(e) {
    if (this.defaults.visibleDataonce < 5 && this.defaults.visibleDataonce > 0) {
      this._showMorePageItems(this.defaults.visibleDataonce);
    } else if (this.defaults.visibleDataonce >= 5) {
      this._showMorePageItems(Math.round(this.defaults.visibleDataonce / 2));
    }
  }
